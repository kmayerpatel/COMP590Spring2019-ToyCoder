...
public class ModelFreeArithmeticDecoder {
	...
	public ModelFreeArithmeticDecoder(int rangeBitWidth) {
	...
	public int decode(double[] pdf, BitSource bit_source) throws IOException, InsufficientBitsLeftException {
		...
		while (true) {
			...
			sym_low = _low + ((long) (range_width * pdf[sym_idx_mid]));
			sym_high = _low + ((long) (range_width * pdf[sym_idx_mid+1])) - 1L;
			
			if (_input_buffer <= sym_high && _input_buffer >= sym_low) {
				range_idx = sym_idx_mid;
				break;
